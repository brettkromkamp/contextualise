...
def edit(map_identifier, topic_identifier, link_identifier):
    ...
    link_occurrence = topic_store.get_occurrence(map_identifier, link_identifier,
                                                 resolve_attributes=RetrievalMode.RESOLVE_ATTRIBUTES)
    ...
    if request.method == 'POST':
        ...
        if error != 0:
            ...
            if link_occurrence.scope != form_link_scope:
                topic_store.update_occurrence_scope(map_identifier, link_occurrence.identifier, form_link_scope)
...
@bp.route('/links/delete/<map_identifier>/<topic_identifier>/<link_identifier>', methods=('GET', 'POST'))
@login_required
def delete(map_identifier, topic_identifier, link_identifier):
    ...
    return render_template('link/delete.html',
                           topic_map=topic_map,
                           topic=topic,
                           link_identifier=link_occurrence.identifier,
                           link_title=form_link_title,
